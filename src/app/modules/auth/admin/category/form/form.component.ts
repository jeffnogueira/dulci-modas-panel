...
export class FormComponent implements OnInit, FormInterface, OnDestroy {
	...
	setOperation(): void {
		...
		if (this.id) {
			...
			this.title = this.operation === Operation.EDIT ? 'Alterar Categoria' : 'Visualizando Categoria';
		} else {
			this.operation = Operation.NEW;
			this.title = 'Incluir Categoria';
		}
	}
	...
	fillForm(): void {
		if (this.operation !== Operation.NEW) {
			this.subscription = this._categoryService.loadOne(Number(this.id)).subscribe((data: any) => {
	...
	onSave(): void {
		if (this._utilsService.formIsValid(this.form)) {
			this._loadingService.show();
			if (this.operation === Operation.NEW) {
				this.subscription = this._categoryService.create(this.form.value).subscribe(data => {
					this._loadingService.hide();
				...
				this.subscription = this._categoryService.update(this.id, this.form.value).subscribe(data => {
					this._loadingService.hide();
