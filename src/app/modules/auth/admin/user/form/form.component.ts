...
export class FormComponent implements OnInit, FormInterface, OnDestroy {
	...
	setOperation(): void {
		...
		if (this.id) {
			...
			this.title = this.operation === Operation.EDIT ? 'Alterar Usuário' : 'Visualizando Usuário';
		} else {
	...
	onSave(): void {
		if (this._utilsService.formIsValid(this.form)) {
			this._loadingService.show();
			if (this.operation === Operation.NEW) {
				this.subscription = this._userService.create(this.form.value).subscribe(data => {
					this._loadingService.hide();
				...
				this.subscription = this._userService.update(this.id, this.form.value).subscribe(data => {
					this._loadingService.hide();
